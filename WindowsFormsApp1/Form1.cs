...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        public Directions course; // направление движения     
        ...
        {
            InitializeComponent();
            course = Directions.LEFT; // начальное направление
            ...
            g = this.gamePanel.CreateGraphics();// создаем графичскую поверхность
        ...
        private void DrawSnake()
        {
            for (int i = 0; i < snake.Count; i++)
            {
                if (i == 0) // рисуем голову 
                    ...
                    g.FillRectangle(Brushes.Black, snake[i]);
                g.DrawRectangle(Pens.Black, snake[i]);
            }
        } //DrawSnake
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            score += scoreDif;
            if (score % 100 == 0) scoreDif += 10;
            scoreLabel.Text = score.ToString();
            timer1.Enabled = false; // останавливаем таймер

            // в зависимости от выбранного направления
            // наращиваем координаты головы (snake[0])
            if (course == Directions.UP)
            {
                addX = 0;
                addY = -20;
            }

            if (course == Directions.DOWN)
            {
                addX = 0;
                addY = 20;
            }

            if (course == Directions.LEFT)
            {
                addX = -20;
                addY = 0;
            }

            if (course == Directions.RIGHT)
            {
                addX = 20;
                addY = 0;
            }
            ...
            if (// выход за границу игрового поля
                (snake[0].X < 0 || snake[0].X > gamePanel.Width ||
                snake[0].Y < 0 || snake[0].Y > gamePanel.Height)
                ||
        ...
        {
            if (e.KeyCode == Keys.Up && course != Directions.DOWN)
            {
                course = Directions.UP;
            }
            if (e.KeyCode == Keys.Down && course != Directions.UP)
            {
                course = Directions.DOWN;
            }
            if (e.KeyCode == Keys.Left && course != Directions.RIGHT)
            {
                course = Directions.LEFT;
            }
            if (e.KeyCode == Keys.Right && course != Directions.LEFT)
            {
                course = Directions.RIGHT;
            }
        }
